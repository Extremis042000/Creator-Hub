...
| `AI_API_BASE_URL` / `AI_API_KEY` / `AI_MODEL` | Optional | blank | openai-compatible provider (FreeModel today; Cloudflare Workers AI / OpenRouter are config-only swaps). **All three required** or no provider is registered. No defaults on purpose — an unvetted third-party host must be named explicitly. Never commit the key. |
