...
| `AI_PROVIDER` | Optional | `openai-compatible` | Which AI backend: `openai-compatible` or `anthropic`. |
| `AI_API_CHAT_URL` / `AI_API_KEY` / `AI_MODEL` | Optional | blank | openai-compatible provider, slot 1 (FreeModel today; Cloudflare Workers AI / OpenRouter are config-only swaps). **All three required** or this slot is inert. `AI_API_CHAT_URL` is the *exact* chat-completion endpoint (conventions differ per gateway — see `AI_API_CHAT_URL_2` below), not just a base. No defaults on purpose — an unvetted third-party host must be named explicitly. Never commit the key. |
