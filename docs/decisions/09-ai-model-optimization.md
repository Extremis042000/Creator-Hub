...
Status: **Phases 35-38 done and live-verified; Phase 39 proposed, not started.**
...
| 37 | First-party usage-signal capture | Phase 28 (`ai_generation_log`) | Medium | ✅ **Done.** `ai_generation_log` gained an `outcome_signal` column (COPIED/REFINED/REGENERATED). REFINED and REGENERATED are inferred entirely server-side from what the user does next (no new UI); COPIED needed one invisible hook on the existing Copy buttons (`onCopy` callback -> fire-and-forget signal call, only when a `refineSessionId` is present, i.e. only for premium AI results). This is the instrumentation Phase 38's admin visibility and any future eval-set growth will read from. |
| 38 | Admin visibility into eval scores + usage signals | Phase 36, 37 | Low | ✅ **Done.** New "AI generation quality signals" admin panel (`GET /api/v1/admin/ai/signals`, all-time -- not scoped to today like the usage card, since signals accumulate too slowly for a daily view to show anything): per-tool successful-generation count plus a Copied/Refined/Regenerated/No-signal-yet segmented bar, sourced live from Phase 37's `outcome_signal` column. The eval-score half is a dated, clearly-labeled static snapshot of Phase 35/36's real numbers (not a live figure -- the eval isn't runtime infrastructure, so there's nothing to recompute without re-running it) with a pointer to the full committed report. 149 backend tests (3 new), `tsc` clean. |
